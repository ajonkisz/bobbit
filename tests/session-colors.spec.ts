...
test.describe("Bobbit session colors palette", () => {
	test("palette has exactly 18 colours after removing indices 8 and 19", async ({ page }) => {
		await page.goto(TEST_PAGE);
		const count = await page.evaluate(() => (window as any).__BOBBIT_HUE_ROTATIONS.length);
		expect(count).toBe(18);
	});

	test("palette does not contain removed hue values 200 or 250", async ({ page }) => {
		await page.goto(TEST_PAGE);
	...
	test("palette retains all other original hue values in order", async ({ page }) => {
		await page.goto(TEST_PAGE);
		const palette = await page.evaluate(() => (window as any).__BOBBIT_HUE_ROTATIONS as number[]);
	...
	test("wraps around when all 18 colours are used", async ({ page }) => {
		await page.goto(TEST_PAGE);
