			...
			${staffSectionExpanded ? list.map((agent) => {
				...
				<div class="${mobile ? "" : "group relative"} flex items-center gap-1 pl-3 pr-1 ${rowPy} rounded-md cursor-pointer transition-colors
					...
					${!mobile && session ? (() => {
						...
						return html`<span class="shrink-0 flex items-center gap-0.5 text-[10px] tabular-nums ${unseen ? "text-muted-foreground font-medium" : "text-muted-foreground/50"}" title="${formatSessionAge(session.lastActivity)}">${time}${unseen ? html`<span class="text-primary" style="font-size:6px;line-height:1;">●</span>` : ""}</span>`;
					})() : ""}
					${mobile
						? editBtn
