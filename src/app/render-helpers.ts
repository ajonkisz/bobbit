	...
	const emptyState = html`
		<div class="pl-2 py-1 ${mobile ? "text-xs" : "text-[11px]"} text-muted-foreground">
			${goal.archived
				...
				: canArchive
				? html`<span style="vertical-align:middle">Work merged —</span> <button class="inline-flex items-center gap-1 px-1.5 py-px rounded bg-destructive/10 text-destructive text-[10px] font-semibold hover:bg-destructive/20 transition-colors align-middle" title="Archive goal" @click=${(e: Event) => { e.stopPropagation(); deleteGoal(goal.id); }}>${icon(Trash2, "xs")}Archive Goal</button>`
				: isTeamGoal
