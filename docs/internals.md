...
### Recommended model tiers (VER-02) - Cheap tier applied, Mid/Frontier remain guidance

Finding VER-02 asked for the model-side companion to the thinkingLevel tiers above: cheap-tier roles running the same frontier model as a team-lead is the single biggest addressable cost lever in verification. The table below is the recommended tiering. As of the D1 follow-up, AJ reviewed the availability-fallback risk described below and approved applying it narrowly to the **Cheap** tier only - `docs-writer` now ships `model: anthropic/claude-haiku-4-5` in `defaults/roles/docs-writer.yaml`. **Mid** and **Frontier** stay guidance-only (apply per-install via the role manager's **Model** tab, or a project/server role-YAML override) - no built-in role in those tiers ships a literal `model:` default. Both the one shipped exception and the two guidance-only tiers are pinned by `tests/builtin-role-model-tiers.test.ts`.

| Recommended tier | Roles | Rationale | Status |
|---|---|---|---|
| Frontier (keep session/review default) | team-lead, architect, security-reviewer, spec-auditor, bug-hunter | Same set as the `high` thinkingLevel tier - orchestration and high-stakes review where a missed finding is expensive. | Guidance only - unchanged |
| Mid | coder, reviewer, code-reviewer, test-engineer, qa-tester | Mechanical-but-technical work; a mid-tier model handles it fine at a fraction of the cost/latency. | Guidance only - implementer/reviewer-class roles intentionally excluded from the shipped default |
| Cheap | docs-writer | Purely mechanical prose/doc edits - the same role F5 already tiers to `thinkingLevel: low`. | **Applied** - `model: anthropic/claude-haiku-4-5` shipped in the built-in role YAML |

**Why Mid/Frontier stay guidance and not a shipped `defaults/roles/*.yaml` value:** `role.model` binding is a hard contract (see Precedence and Failure handling below) - a `setModel` failure or read-back mismatch throws, and only falls back to `default.sessionModel` when the operator has separately opted into `allowSessionModelFallback` (off by default; see [Controlled session model fallback](session-model-fallback.md)). A literal `<provider>/<modelId>` string is provider-specific: shipping one as a role default would hard-fail every spawn of that role on any install that only has an AI Gateway, Bedrock, or a different provider configured - the opposite of graceful degradation, and a much worse outcome than "runs at the frontier price." `tests/controlled-model-fallback.test.ts` ("off/absent setting: failing explicit role.model rejects...") pins exactly this risk. Until Bobbit has a symbolic, install-portable tier (a value that resolves against whatever the operator actually has configured, the way `selectAigwModelForRoleTier` already does for the AI-Gateway auto-select path below), a hardcoded literal model default is not safe to ship broadly for Mid/Frontier roles.

**Why Cheap was applied anyway:** AJ accepted the same availability-fallback risk for `docs-writer` alone, because (a) it's the single "purely mechanical" role in the table, already forbidden by its own prompt from touching runtime logic, so a wrong/slow model has low blast radius; (b) an operator whose install lacks `anthropic/claude-haiku-4-5` can override `docs-writer`'s model per-role (role manager **Model** tab or a project/server role-YAML override) without touching any other role or the fallback contract; and (c) this keeps the change scoped to one role rather than extending the risk to the five Mid-tier implementer/reviewer roles or the five Frontier-tier roles, which all remain guidance-only. **This has NOT been generalized to a symbolic/install-portable tier** - installs without that exact Anthropic model configured (Bedrock-only, AI-Gateway-only with a different catalog, etc.) will hard-fail `docs-writer` spawns unless they override it. `tests/builtin-role-model-tiers.test.ts` pins this as a deliberate, narrow exception, not a precedent for the other tiers.
